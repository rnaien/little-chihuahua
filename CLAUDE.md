...
- Every page shares the same `<head>` boilerplate: UTF-8 meta, an inline SVG data-URI paw favicon, Google Fonts preconnect + `Baloo 2`/`Inter` stylesheet link (switched from Fredoka, whose font file is missing the Turkish `ş`/`ğ` glyphs — confirmed via cmap inspection), and `<link rel="stylesheet" href="kopek-rehberi.css">`. Keep new pages consistent with this pattern rather than inlining styles or fonts.
